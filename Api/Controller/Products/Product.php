...
class Product {
    ...
    public static function findUsersProducts(int $currentUser){
        $product = (new ProductModel)->findAllUsersProducts($currentUser, ['id','brand_id', 'name', 'type']);
        if($product) {
            return $product;
        }
    ...
    public static function findProduct(Request $request){
        $product = (new ProductModel)->findProduct($request->currentUser, $request->getPathParams()['id'], ['id', 'account_id','brand_id', 'name', 'type']);
        if($product) {
            return $product;
        }
        Exception::throw("Product not found", 200);
    ...
    public static function updateProduct(Request $request){
        $product = (new ProductModel)->findProduct($request->currentUser, $request->getPathParams()['id'], ['id','account_id', 'name', 'type'], false);
        if($product) {
